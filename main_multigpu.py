...
def ddp_prepare(rank, world_size):
    train_dataset = WatermarkDataset(TRAIN_PATH)
    sampler = DistributedSampler(train_dataset, num_replicas=world_size, rank=rank, shuffle=True)
    train_dataloader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=0, sampler=sampler, pin_memory=False)
...
def ddp_main(rank, world_size):
    ...
    train_dataloader = ddp_prepare(rank, world_size)
...
if __name__ == "__main__":
    mp.spawn(ddp_main, args=(NUM_GPUS, ), nprocs=NUM_GPUS)